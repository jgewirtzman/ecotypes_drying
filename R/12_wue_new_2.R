...
final_plot_wue <- (iwue_plot / wue_plot / iwue_star_plot) +
...
# Combine scatter plots with shared legend
wue_scatter_panel <- (p_iwue_wue | p_iwue_star) / (p_wue_star | ggplot2::ggplot() + theme_void()) +
  patchwork::plot_layout(guides = "collect") &
  theme(legend.position = "top")

print(wue_scatter_panel)

# ============================================================================ #
# HELPER FUNCTIONS FOR COMPARISONS
# ============================================================================ #
mean_cld_table <- function(model, metric_name) {
  em <- emmeans::emmeans(model, ~ Population * TreatmentName)
  cld_obj <- multcomp::cld(em, Letters = letters)
  cld_obj %>%
    as.data.frame() %>%
...
pairwise_contrasts <- function(model, contrast_type, metric_name) {
  if (contrast_type == "within_pop") {
    em <- emmeans::emmeans(model, ~ TreatmentName | Population)
  } else if (contrast_type == "within_trt") {
    em <- emmeans::emmeans(model, ~ Population | TreatmentName)
  } else {
    stop("contrast_type must be 'within_pop' or 'within_trt'")
  }
...
# ============================================================================ #
# FINAL COMBINED FIGURE WITH PLOT LETTERS
# ============================================================================ #

# Create side-by-side layout: WUE plots on left, isotope plots on right
# WITH ENHANCED PLOT LETTER FORMATTING
